...
public final class PlacementMirrorPredictionRenderer {
    ...
    private void renderBlockAt(ItemStack block, BlockPos pos, BlockPos last) {
        if (block.getItem() == null) return;
        GL11.glPushMatrix();
        GL11.glTranslated(
                pos.getX() + 0.5 - Minecraft.getMinecraft().getRenderManager().renderPosX,
                pos.getY() + 0.5 - Minecraft.getMinecraft().getRenderManager().renderPosY,
                pos.getZ() + 0.5 - Minecraft.getMinecraft().getRenderManager().renderPosZ);

        GL11.glColor4f(1F, 1F, 1F, 0.6F);
        Minecraft.getMinecraft().renderEngine.bindTexture(TextureMap.LOCATION_BLOCKS_TEXTURE);
        //blockRender.use = false;
        blockRender.renderBlock(Block.getBlockFromItem(block.getItem()).getStateFromMeta(block.getItemDamage()), 1F);
